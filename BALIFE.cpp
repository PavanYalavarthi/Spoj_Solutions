/*
Problem Credits: https://www.spoj.com/problems/BALIFE/
...
/*
Solution:
    Its more like load is shifted from one part to another, 
    So we are seeing how much max can be transfered from one part to rest.
*/
#include<bits/stdc++.h>
using namespace std;
...
int main() {
    while(true) {
        ...
        long ans = -1;
        if (v.back() % N == 0) {
            // Processing only if total is divisible by N
            long value_per_cell =  v.back() / N;
            for(int i = 0; i < N; i++) {
                ans = max(ans, abs((i + 1) * value_per_cell - v[i]));
            }
        }
        cout << ans << endl;
    }
